...
import type { BomCatOverride, BOMResult, BomOverride, Cell, CellType, ConfigState, Grid } from '@/core/types';
...
const DEFAULT: ConfigState = {
  ...
  depthLayers: 1,
  grid:        [[[{ type: 'O', frameGroup: '', shelves: 0 }]]],
  profileColor: 'SW',
  footer:       'stell_m6',
  opts: {
    footer:   true,
...
export interface ConfigActions {
  ...
  toggleOpt(k: 'outer' | 'inner' | 'back'): void;
  ...
  setFrameGroup(r: number, c: number, d: number, group: string): void;
  setBomOverride(id: string, override: BomOverride): void;
  clearBomOverride(id: string): void;
  ...
  setCellColor(row: number, col: number, hex: string): void;
  clearCellColor(row: number, col: number): void;
  setCableHole(boardId: string, value: boolean): void;
  setBomOverrideByBoard(boardId: string, material: string, color: string): void;
  clearBomOverrideByBoard(boardId: string): void;
  addColLeft(): void;
  removeColLeft(): void;
  addColRight(): void;
  removeColRight(): void;
...
export function useConfigStore(): [ConfigState, ConfigActions, () => void] {
  ...
  const actions: ConfigActions = {
    ...
    // ── Globale Parameter ─────────────────────────────────────────────────
    setDepth: (d) => update(s => ({ ...s, depth: d, depthLayers: Math.round(d / ELEMENT_SIZE_MM) })),

    setSurface: (v) => update(s => ({
      ...s, profileColor: v, surface: v, cellColors: {}, partColors: {},
    })),

    setProfileColor: (v) => update(s => ({
      ...s, profileColor: v, surface: v, cellColors: {}, partColors: {},
    })),

    setHandle: (v) => update(s => ({ ...s, handle: v })),
    setFooter: (v) => update(s => ({ ...s, footer: v })),

    toggleOpt: (k) => update(s => ({
      ...s, opts: { ...s.opts, [k]: !s.opts[k as keyof typeof s.opts] },
    })),

    // ── Zelltyp setzen ───────────────────────────────────────────────────
    setType: (r, c, t) => {
      // Lightmodul: keine Schwerkraft-Regel
      ...
      update(s => {
        ...
        const grid: Grid = s.grid.map((rowArr, ri) =>
          rowArr.map((colArr, ci) => {
            ...
            return colArr.map((cell, d) => {
              // Setze Typ auf allen Tiefenebenen oder nur d=0?
              // Standard: alle Tiefenebenen gleichzeitig setzen (konsistente Struktur)
              const max = maxShelves(s.rows[r]);
              const shelves = Math.min(cell.shelves, max);
              return { ...cell, type: t, shelves };
            });
          })
        );
        return { ...s, grid };
    ...
    // ── Produktgruppe für Einlegerahmen setzen ───────────────────────────
    setFrameGroup: (r, c, d, group) => update(s => {
      const grid: Grid = s.grid.map((rowArr, ri) =>
        rowArr.map((colArr, ci) => {
          if (ri !== r || ci !== c) return [...colArr];
          return colArr.map((cell, di) => {
            if (di !== d) return cell;
            return { ...cell, frameGroup: group };
          });
        })
      );
      return { ...s, grid };
    ...
    setDepthLayers: (d) => update(s => {
      ...
      return { ...s, depthLayers: nD, depth: nD * ELEMENT_SIZE_MM, grid };
    }),

    // ── BOM-Overrides ─────────────────────────────────────────────────────
    setBomOverride: (id, override) => update(s => ({
      ...s, bomOverrides: { ...s.bomOverrides, [id]: override },
    })),
    clearBomOverride: (id) => update(s => {
      const next = { ...s.bomOverrides }; delete next[id];
      return { ...s, bomOverrides: next };
    }),
    setCatOverride: (catKey, override) => update(s => ({
      ...s, catOverrides: { ...s.catOverrides, [catKey]: override },
    })),
    clearCatOverride: (catKey) => update(s => {
      const next = { ...s.catOverrides }; delete next[catKey];
      return { ...s, catOverrides: next };
    }),
    setPartColor: (plateId, hex) => update(s => ({
      ...s, partColors: { ...s.partColors, [plateId]: hex },
    })),
    clearPartColor: (plateId) => update(s => {
      const pc = { ...s.partColors }; delete pc[plateId];
      return { ...s, partColors: pc };
    }),
    clearAllPartColors: () => update(s => ({ ...s, partColors: {} })),
    setCellColor: (row, col, hex) => update(s => ({
      ...s, cellColors: { ...s.cellColors, [`${row}_${col}_0`]: hex },
    })),
    clearCellColor: (row, col) => update(s => {
      const cc = { ...s.cellColors }; delete cc[`${row}_${col}_0`];
      return { ...s, cellColors: cc };
    }),
    setCableHole: (boardId, value) => update(s => ({
      ...s, cableHoles: { ...s.cableHoles, [boardId]: value },
    })),
    setBomOverrideByBoard: (boardId, material, color) => update(s => ({
      ...s, bomOverrides: { ...s.bomOverrides, [boardId]: { material, color } },
    })),
    clearBomOverrideByBoard: (boardId) => update(s => {
      const next = { ...s.bomOverrides }; delete next[boardId];
      return { ...s, bomOverrides: next };
    }),
